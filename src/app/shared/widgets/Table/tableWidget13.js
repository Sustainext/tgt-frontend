...
const CustomTableWidget13 = ({ id, options, value, required, onChange }) => {
  ...
  return (
    <div style={{ maxHeight: "400px" }} className="mb-2 w-full overflow-x-auto">
      <table
        id={id}
      ...
      >
        <thead className="gradient-background h-[54px]">
          <tr>
            ...
            {options.titles.map((item, idx) => (
              <th
                key={idx}
                className="text-[12px] border-l border-gray-300 px-2 py-2 w-[35%] xl:w-[35%] text-center"
              >
                {item.title}
              </th>
            ))}
        ...
        <tbody>
          {options.rowLabels.map((rowLabel, rowIndex) => (
            <tr key={rowIndex}>
              <td className="border-t flex border-gray-300 py-2 px-2 text-[13px] w-[280px] xl:w-auto lg:w-auto md:w-auto 4k:w-auto 2k:w-auto relative">
                <p>{rowLabel.title}</p>
                <MdInfoOutline
              ...
              {options.titles.map((column, columnIndex) => (
                <td
                  key={columnIndex}
                  className="border-t border-l border-gray-300"
                >
                  <input
                    type={column.type}
                    required={required}
