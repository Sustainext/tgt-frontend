...
const CustomTableWidget3 = ({ id, options, value, required, onChange }) => {
    ...
    const computeRowTotal = (rowIndex, values) => {
        ...
        return options.titles.reduce((acc, column) => {
            const value = rowValues && rowValues[column.key] ? parseInt(rowValues[column.key], 10) : 0;
            return acc + value;
        }, 0);
    };
        ...
        <div style={{  maxHeight: "400px" }} className='mb-2'>
            <table id={id} className="rounded-md border border-gray-300 w-full">
                <thead className="gradient-background">
                    <tr>
                    <th className="text-[12px] border border-gray-300 px-2 py-2 w-auto text-left">Employee Category </th>
                        ...
                        <th className="text-[12px] border border-gray-300 px-2 py-2 w-auto text-center">Total</th>
                    </tr>
                </thead>
                <tbody>
                    {options.rowLabels.map((rowLabel, rowIndex) => (
                        <tr key={rowIndex}>
                            ...
                            <td className="border border-gray-300 text-center text-sm"> {localValue[rowIndex].total}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
