...
const InputField = React.memo(({ type, required, value, onChange }) => {
    const handleInputChange = (e) => {
        ...
        onChange(e.target.type === 'checkbox' ? e.target.checked : e.target.value, type);
    };
...
const CustomTableWidget4 = ({ id, options, value, required, onChange, formContext }) => {
    ...
    return (
        <div style={{ overflowY: "auto", maxHeight: "600px" }}>
            <table id={id} style={{ width: "100%", maxWidth: "1010px", minWidth: "300px", overflowY: "auto", maxHeight: "400px" }} className="rounded-md border border-gray-300">
                    ...
                    {localData.map((item, rowIndex) => (
                        <tr key={rowIndex}>
                            {Object.keys(item).map((key, cellIndex) => (
                                <td key={cellIndex} className="border border-gray-300 p-3 w-[30%]">
                                    <InputField
