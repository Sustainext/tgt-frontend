        ...
        <thead className="gradient-background h-[54px]">
          <tr>
            {uiSchema["ui:options"].titles.map((item, idx) => (
              ...
              >
                <div className="flex justify-center relative  w-[250px] xl:w-auto lg:w-auto  md:w-auto  2xl:w-auto  4k:w-auto  2k:w-auto">
                  <p>{item.title}</p>
                  <MdInfoOutline
          ...
          {value.map((item, rowIndex) => (
            <tr key={rowIndex}>
              {Object.keys(item).map((key, cellIndex) => {
                ...
                const inputType =
                  uiSchema["ui:options"].titles.find((t) => t.title2 === key)
                    ?.type || "text";
                    ...
                    {isEnum ? (
                      <>
                        <select
                          value={item[key]}
                          onChange={(e) =>
                            updateField(rowIndex, key, e.target.value, isEnum)
                          }
                      ...
                      <>
                        {key === "totalnumberofincidentsofdiscrimination" ||
                        key === "describetheincident" ? (
                          <>
                            <input
                              type={inputType}
                              required={required}
