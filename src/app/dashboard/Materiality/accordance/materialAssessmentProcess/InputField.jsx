...
const InputField = ({ handleTabClick }) => {
    ...
    items: {
      ...
      properties: {
        Q1: {
          type: "string",
          title:
            "How the organisation has identified actual and potential, negative and positive impacts on the economy, environment and people including impacts on their human rights, across its activities and business relationships?",
        },
        Q2: {
          type: "string",
          title:
            "How has the organization prioritized impacts for reporting based on their significance?",
        },
        Q3: {
          type: "string",
          title:
            "Specify the stakeholders and experts whose views have informed the process of determining organisation's material topics.",
  ...
  const handleSubmit = async (e) => {
    ...
    const url = dataPresent
      ? `${process.env.BACKEND_API_URL}/materiality_dashboard/materiality-assessment-process/${assessment_id}/`
      : `${process.env.BACKEND_API_URL}/materiality_dashboard/materiality-assessment-process/create/`;
    try {
    ...
    <>
      <div className="mt-7">
        <div className="flex  xl:flex-row md:flex-row lg:flex-row 4k:flex-row 2k:flex-row  flex-col items-center mb-2 xl:gap-2 gap-4">
          <div className="flex items-start  xl:w-[54.2%] md:w-[55%] lg:w-[55%] 4k:w-[55%] 2k:w-[55%] w-full px-2">
            <p className="text-[15px] text-gray-700 w-auto">
              Describe the process organisation has followed to determine its
