...
const AnalyseWaste = ({ isBoxOpen }) => {
  ...
  const fetchData = async (params) => {
    ...
    try {
      ...
      const { waste_generated_by_material,waste_generated_by_location} = data;
      ...
      const waste_generated_by_material_total = removeAndStoreLastObject(waste_generated_by_material);
      const roundedWasteGeneratedByMaterial = waste_generated_by_material.map((item) => ({
        ...item,
      ...
      roundedWasteGeneratedByMaterial.push({
        ...
        contribution: "",
        totalrow: 2,
        total_waste: waste_generated_by_material_total.total_waste_generated.toFixed(2),
        units: "t (metric tons)",
      });

      setWastedata1(roundedWasteGeneratedByMaterial);

      const waste_generated_by_location_total = removeAndStoreLastObject(waste_generated_by_location);
      const wastegeneratedbylocation = waste_generated_by_location.map((item) => ({
        ...item,
        total_waste: item.total_waste.toFixed(2),
      }));
      wastegeneratedbylocation.push({
        ...
        contribution: "",
        totalrow: 2,
        total_waste: waste_generated_by_location_total.total_waste_generated.toFixed(2),
        units: "t (metric tons)",
      });
      setWastedata2(wastegeneratedbylocation);

      const nonhazardous_total = removeAndStoreLastObject(hazardous_and_non_hazardous_waste_composition);
      const wastegeneratedbycategory = hazardous_and_non_hazardous_waste_composition.map((item) => ({
        ...item,
        total_waste: item.total_waste.toFixed(2),
      }));
      ...
      console.log(wastegeneratedbycategory,"test data")

      const waste_directed_to_disposal_by_material_type_total = removeAndStoreLastObject(waste_directed_to_disposal_by_material_type);
      const wastedirectedtodisposalbymaterialtype = waste_directed_to_disposal_by_material_type.map((item) => ({
        ...item,
        total_waste: item.total_waste.toFixed(2),
      }));
      wastegeneratedbylocation.push({
        ...
        totalrow: 3,
        maprow:1,
        total_waste: waste_directed_to_disposal_by_material_type_total.total_waste_generated.toFixed(2),
        units: "t (metric tons)",
      });
