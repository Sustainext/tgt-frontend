          ...
          {data.length === 0 || isEmptyData ? (
            ...
            <>
              {data.map((location, locationIndex) => {
                const locationKey = Object.keys(location)[0]; // Get the dynamic location key
                const employees = location[locationKey]; // Get the array of employees for this location
                  ...
                  <React.Fragment key={locationIndex}>
                    {employees.map((employee, employeeIndex) => (
                      <tr key={`${locationIndex}-${employeeIndex}`} className="md:table-row">
                        ...
                        {columns.slice(1).map((columnKey, colIndex) => (
                          ...
                          >
                            {colIndex === 3  // Assuming index 5 (colIndex 3 after slice) is the percentage column
                    ...
                    <tr className="md:table-row border-r border-l border-gray-300">
                      ...
                      <td className="p-2 block md:table-cell text-center text-sm border border-gray-300">
                        {employees.reduce((acc, employee) => acc + (employee[columns[2]] || 0), 0)}
                      </td>
                      {/* Column 4 */}
                      <td className="p-2 block md:table-cell text-center text-sm border border-gray-300">
                        {employees.reduce((acc, employee) => acc + (employee[columns[3]] || 0), 0)}
                      </td>
                      {/* Column 5 (calculated as (column 3 / column 4) * 100 with % symbol) */}
                      <td className="p-2 block md:table-cell text-center text-sm border border-gray-300">
                        {(() => {
                          const totalColumn3 = employees.reduce((acc, employee) => acc + (employee[columns[2]] || 0), 0);
                          const totalColumn4 = employees.reduce((acc, employee) => acc + (employee[columns[3]] || 0), 0);
