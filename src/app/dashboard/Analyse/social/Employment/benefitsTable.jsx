          ...
          {locationdata.map((location, rowIndex) => (
            <tr key={rowIndex}>
              ...
              {data.map((header, colIndex) => {
                const isSelected = header.selected && header.selected.includes(location.location_id);
                return (
                  <td key={colIndex} className="px-6 py-4 border-b border-gray-200 text-center">
                    {isSelected ? (
