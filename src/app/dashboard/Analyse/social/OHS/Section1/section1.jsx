...
const Screen1 = ({ isBoxOpen,location,year }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axiosInstance.get(
        `/sustainapp/get_ohs_analysis?location=${location}&start=${year}-01-01&end=${year}-12-31`,
      ...
      function formatArray4(operations) {
        return operations.map((operation, index) => ({
          ...
          "Rate of high-consequence work-related injuries (excluding fatalities)":
            Number(
              operation.rate_of_high_consequence_work_related_injuries_excluding_fatalitie
            ),
          "Rate  of recordable work-related injuries": Number(
            operation.rate_of_recordable_work_related_injuries
          ),
