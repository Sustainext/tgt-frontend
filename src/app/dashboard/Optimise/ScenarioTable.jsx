...
const ScenarioTable = () => {
  ...
  const handleConfirmDelete = () => {
    ...
    toast.success(`Scenario "${scenarioToDelete.name}" has been deleted successfully`, {
        position: "top-right",
        autoClose: 3000,
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (baseYearFilterRef.current && !baseYearFilterRef.current.contains(event.target)) {
        setIsBaseYearFilterOpen(false);
      }
      if (targetYearFilterRef.current && !targetYearFilterRef.current.contains(event.target)) {
        setIsTargetYearFilterOpen(false);
      }
      if (corpFilterRef.current && !corpFilterRef.current.contains(event.target)) {
  ...
  const FilterModal = ({ title, items, selectedItems, handleSelection, onClose }) => {
    return (
          ...
          <div className="space-y-2 max-h-60 overflow-y-auto">
            {items.map((item) => (
              ...
              >
                <input
                  type="checkbox"
                  checked={selectedItems.includes(typeof item === 'object' ? item.name || item.label : item)}
                  onChange={() => handleSelection(typeof item === 'object' ? item.name || item.label : item)}
  ...
  const EmptyState = () => (
    <div className="flex flex-col items-center justify-center py-16 my-8">
      ...
      <h2 className="text-2xl font-semibold text-gray-800 mb-4">No Scenarios Present</h2>
      <p className="text-center text-gray-600 mb-8 max-w-lg">
        Define your goals, set metrics, and visualize your path to achieving net-zero emissions. Click the 
  ...
  return (
    ...
    <div>
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Scenario Creation Dashboard</h1>
          <p className="text-gray-500 mt-1">
            Create new scenarios, view existing ones, and make adjustments to your plans. Track your progress, compare different strategies, and visualize your path to achieving net-zero goals.
          </p>
        </div>
            ...
            <div className="grid grid-cols-9 gap-4 px-6 py-4 border-b border-gray-200 text-sm font-medium text-gray-500">
              ...
              <div className="col-span-1 relative" ref={baseYearFilterRef}>
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => setIsBaseYearFilterOpen(!isBaseYearFilterOpen)}
                    className="hover:bg-gray-100 p-1 rounded"
                  >
                    <MdFilterList className={`w-4 h-4 ${selectedBaseYears.length > 0 ? 'text-blue-500' : 'text-gray-400'} hover:text-blue-500`} />
                  </button>
                </div>
              ...
              <div className="col-span-1 relative" ref={targetYearFilterRef}>
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => setIsTargetYearFilterOpen(!isTargetYearFilterOpen)}
                    className="hover:bg-gray-100 p-1 rounded"
                  >
                    <MdFilterList className={`w-4 h-4 ${selectedTargetYears.length > 0 ? 'text-blue-500' : 'text-gray-400'} hover:text-blue-500`} />
                  </button>
                </div>
              ...
              <div className="col-span-1 relative" ref={orgFilterRef}>
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="hover:bg-gray-100 p-1 rounded"
                  >
                    <MdFilterList className={`w-4 h-4 ${selectedOrganizations.length > 0 ? 'text-blue-500' : 'text-gray-400'} hover:text-blue-500`} />
                  </button>
                </div>
              ...
              <div className="col-span-1 relative" ref={corpFilterRef}>
                <div className="flex items-center gap-2">
                  <span>Corporate</span>
                  <button
                    onClick={() => setIsCorporateFilterOpen(!isCorporateFilterOpen)}
                    className="hover:bg-gray-100 p-1 rounded"
                  >
                    <MdFilterList className={`w-4 h-4 ${selectedCorporates.length > 0 ? 'text-blue-500' : 'text-gray-400'} hover:text-blue-500`} />
                  </button>
                </div>
              ...
              {isLoading ? (
                ...
                scenarios.map((scenario) => (
                  ...
                  >
                    ...
                    <div className="col-span-1">{scenario.organization}</div>
                    <div className="col-span-1">{scenario.corporate}</div>
                    
                    <div className="col-span-1 text-gray-500">
                      <div>{scenario.createdBy}</div>
                      <div className="text-xs">
                        <Moment format="DD/MM/YYYY">{scenario.created}</Moment>
                      </div>
                    </div>
                    ...
                    <div className="col-span-1 text-gray-500">
                      ...
                      <div className="text-xs">
                        <Moment format="DD/MM/YYYY">{scenario.edited}</Moment>
                      </div>
                    </div>
                    
                    <div className="col-span-1 flex justify-end gap-3">
                      <button className="text-gray-500 hover:text-blue-600" title="View" onClick={() => handleViewScenario(scenario)}
>
                        <FiEye size={18} />
                      </button>
                      <button className="text-gray-500 hover:text-indigo-600" title="Edit">
                        <FiEdit size={18} />
                      </button>
                      <button className="text-gray-500 hover:text-red-600" title="Delete" onClick={() => handleDeleteClick(scenario)}
>
                        <FiTrash2 size={18} />
                      </button>
                    </div>
