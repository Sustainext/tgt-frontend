...
const ComparisonView = ({ 
  ...
  const [showTrendDownloadOptions, setShowTrendDownloadOptions] = useState(false);
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (trendDownloadButtonRef.current && trendDownloadOptionsRef.current &&
      ...
      if (gapDownloadButtonRef.current && gapDownloadOptionsRef.current &&
  ...
  const handleTrendDownloadCSV = useCallback((e) => {
    ...
    try {
      ...
      a.setAttribute("download", `emissions-trend-${scenarioNames}-${timestamp}.csv`);
  ...
  const handleGapDownloadCSV = useCallback((e) => {
    // Stop propagation to prevent dropdown from closing immediately
    e.stopPropagation();
    ...
    try {
      ...
      a.setAttribute("hidden", "");
      a.setAttribute("href", url);
      a.setAttribute("download", `emissions-gap-${scenarioNames}-${timestamp}.csv`);
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
  ...
  const handleTrendDownloadPNG = useCallback((e) => {
    // Stop propagation to prevent dropdown from closing immediately
    e.stopPropagation();
    ...
    try {
      ...
      const paddingTop = 80;
      const paddingBottom = 120; // Increased bottom padding for scenario label
      ...
      titleText.textContent = "Emissions Trend Comparison";
      ...
      subtitleText.setAttribute("y", newHeight - paddingBottom/2 + 50); // Position below title
      ...
      subtitleText.setAttribute("fill", "#777");
      ...
      infoText.setAttribute("y", newHeight - paddingBottom/2 + 70); // Position below subtitle
  ...
  }, [trendChartContainerRef, selectedScenarios, allScenarios]);

  // Handle PNG download for gap chart
  const handleGapDownloadPNG = useCallback((e) => {
    // Stop propagation to prevent dropdown from closing immediately
    e.stopPropagation();
    ...
    try {
      ...
      titleText.setAttribute("font-weight", "bold");
      titleText.setAttribute("fill", "#555");
      titleText.textContent = "Emissions Gap Analysis";
      svgClone.appendChild(titleText);
        ...
        <div className="p-6">
          ...
          <div className="mb-8">
            ...
            {comparisonData && Object.keys(comparisonData).length > 0 && emissionsLineData.length > 0 ? (
              <div ref={trendChartContainerRef} className="emission-chart-container">
                <EmissionsTrendChart 
              ...
              <div className="text-center py-10 bg-gray-50 rounded-md border border-gray-100">
                ...
                <p className="text-gray-500">No emissions trend data available.</p>
          ...
          <div className="mt-10">
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="relative" ref={gapDownloadButtonRef}>
                <button
                  ...
                  className="inline-flex items-center px-3 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50"
                  disabled={isLoading || !comparisonData || emissionsGapData.length === 0 || isDownloading}
                >
                  {isDownloading ? "Processing..." : "Download"}
                  <FiDownload className="ml-2 h-4 w-4" />
                </button>
            ...
            {comparisonData && Object.keys(comparisonData).length > 0 && emissionsGapData.length > 0 ? (
              <div ref={gapChartContainerRef} className="emission-chart-container">
                <EmissionsGapChart 
              ...
              <div className="text-center py-10 bg-gray-50 rounded-md border border-gray-100">
                <FiInfo className="h-12 w-12 text-gray-400 mx-auto mb-4" />
                <p className="text-gray-500">No emissions gap data available.</p>
                <p className="text-sm text-gray-400 mt-1">
                  {selectedScenarios.length === 0 
                    ? "Please select scenarios to compare." 
