...
const DynamicTable2 = ({ data, columns, title }) => {
  ...
  return (
    ...
    >
      <table
        className="min-w-full w-full rounded-lg border border-gray-300 "
        style={{ borderCollapse: "separate", borderSpacing: 0 }}
      >
        <thead className=" md:table-header-group gradient-background">
          ...
          <tr className="md:table-row  border border-gray-300">
            {columns.map((column, index) => (
              <th
                key={index}
                className={`px-4 py-4  text-[#727272]   md:table-cell text-center text-[12px]   border-gray-300 ${
                  index === 0 ? "" : "border-l"
                } `}
                style={
                  index === 0 || index === 1
                    ? { width: "11rem", textAlign: "center" }
                    : { textAlign: "center" }
                }
              >
                {column}
          ...
          {isEmptyData ? (
            <tr className="border-t border-gray-300">
              <td
                ...
                className="text-center p-4  md:table-cell text-[12px] font-normal text-slate-500 border-t border-gray-300"
              >
                No data available
              </td>
            </tr>
                ...
                <React.Fragment key={locationIndex}>
                  {employees.length > 0 ? (
                    employees.map((employee, employeeIndex) => (
                      <tr
                        key={`${locationIndex}-${employeeIndex}`}
                      ...
                      >
                        ...
                        {employeeIndex === 0 && (
                          <td
                            ...
                            className="p-4  md:table-cell text-center text-[12px] border-r border-t  border-gray-300"
                          >
                            {location}
                          </td>
                        )}
                        {columns.slice(1).map((columnKey, colIndex) => (
                          <td
                            key={colIndex}
                            className={`p-4  md:table-cell text-center text-[12px] font-normal text-slate-500  border-t  border-gray-300 ${
                              colIndex === 0 ? "" : "border-l"
                            }`}
                          >
                    ...
                    <tr key={locationIndex} className="md:table-row">
                      <td
                        colSpan={columns.length}
                        className="text-center p-4  md:table-cell text-[12px] font-normal text-slate-500"
                      >
                  ...
                  {employees.length > 0 && (
                    <tr className="md:table-row border-r border-l border-gray-300">
                      ...
                      <td className="p-4  total md:table-cell text-center text-[12px] font-bold border-r border-t border-gray-300">
                        Total
                      </td>
                      {/* Column 3 */}
                      <td className="p-4  md:table-cell text-center text-[12px] border-t border-gray-300">
                        {employees.reduce(
                          (acc, employee) =>
                            acc + parseFloat(employee[columns[2]] || 0),
                          0
                        )}
                      </td>
                      {/* Column 4 */}
                      <td className="p-4  md:table-cell text-center text-[12px] border-l border-t border-gray-300">
                        {employees.reduce(
                          (acc, employee) =>
                            acc + parseFloat(employee[columns[3]] || 0),
                          0
                        )}
                      </td>
                      ...
                      <td className="p-4  md:table-cell text-center text-[12px] border-l border-t border-gray-300">
                        {(() => {
                          const totalColumn3 = employees.reduce(
                            (acc, employee) =>
                              acc + parseFloat(employee[columns[2]] || 0),
                            0
                          );
                          const totalColumn4 = employees.reduce(
                            (acc, employee) =>
                              acc + parseFloat(employee[columns[3]] || 0),
                            0
                          );
