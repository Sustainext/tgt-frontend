...
const Section29 = ({ section12_5_6Ref, orgName, data, reportType }) => {
  ...
  const loadContent = () => {
    dispatch(
      setSignificantSpills(
        ...
        } organization is committed to preventing and managing significant spills that can negatively impact the environment, biodiversity, and local communities.As part of our sustainability strategy, we have implemented a`
      )
    );
  };
  ...
  const col2 = ["Location of spill", "Volume of the spill", "Unit"];
  ...
  const TableData1 =
    data && data["waste_analyse"]
      ? data["waste_analyse"]["total_number_and_volume_by_material"]
        ? data["waste_analyse"]["total_number_and_volume_by_material"].length >
          0
          ? data["waste_analyse"]["total_number_and_volume_by_material"].map(
              (val) => {
                return {
                  ...
                  Unit: val.unit,
                  "Impacts of Significant Spill":
                    val.impact && val.impact.length > 0
                      ? (() => {
                          const filteredImpact = val.impact.filter(
                            (item) => item !== "N/A"
                          );

                          if (filteredImpact.length === 0) {
                            return (
                              <span>This spill has no significant impact.</span>
                            );
                          } else {
  ...
  const TableData2 =
    data && data["waste_analyse"]
      ? data["waste_analyse"]["total_number_and_volume_by_location"]
        ? data["waste_analyse"]["total_number_and_volume_by_location"].length >
          0
          ? data["waste_analyse"]["total_number_and_volume_by_location"].map(
              (val) => {
                return {
  ...
  const TableData3 =
    data && data["waste_analyse"]
      ? data["waste_analyse"]["total_number_and_volume_significant_spills"]
        ? data["waste_analyse"]["total_number_and_volume_significant_spills"]
            .length > 0
          ? data["waste_analyse"][
              ...
              return {
                "Total number of Significant spill":
                  val.number_of_significant_spills,
      ...
      <div id="section12_5_6" ref={section12_5_6Ref}>
        ...
        <div className="xl:flex lg:flex md:flex 4k:flex 2k:flex 2xl:flex justify-between">
          <p className="text-[15px] text-[#344054] mb-2 mt-3">
            Add statement about company’s programs for preventing and managing
            significant spills
          </p>
          <button
