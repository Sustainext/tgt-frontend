  ...
  const RiskTable = ({ title, riskData, riskType }) => {
    const getColumns = () => {
      if (riskType === 'physical') {
        return [
          "Risk Category", "Type of Risk", "Severity of Risk", "Potential Impact", 
          "Likelihood of Impact", "Magnitude of Impact", "Financial Effect", 
          "Financial Implications", "Process Description", "Management Methods", 
          "Time Frame", "Direct or Indirect Impacts", "Implemented Mitigation Strategies", 
          "Mitigation Strategies"
        ];
      } else if (riskType === 'other') {
        return [
          "Risk Category", "Type of Risk", "Potential Impact", "Likelihood of Impact", 
          "Magnitude of Impact", "Financial Effect", "Financial Implications", 
          "Management Methods", "Time Frame", "Direct or Indirect Impacts", 
          "Mitigation Strategies Implementation", "Mitigation Strategies"
        ];
      } else {
        return [
          "Risk Category", "Type of Risk", "Potential Impact", "Likelihood of Impact", 
          "Magnitude of Impact", "Financial Effect", "Financial Implications", 
          "Process Description", "Management Methods", "Time Frame", 
          "Direct or Indirect Impacts", "Implemented Mitigation Strategies", 
          "Mitigation Strategies"
        ];
      }
    };
              ...
              <thead>
                <tr className="bg-gradient-to-r from-sky-500/5 to-lime-500/5">
                  {columns.map((col, index) => (
                    <th key={index} className="border-r border-gray-200 py-8 px-4 text-left text-gray-600 font-medium">
                      {col}
                    </th>
                  ))}
                </tr>
                ...
                {riskData?.map((risk, index) => (
                  <tr key={index} className="bg-white border-t border-gray-200 hover:bg-gray-50 transition-colors">
                    <td className="border-r border-gray-200 p-4 text-gray-700">
                      ...
                      {riskType === 'other' && (risk.RiskCategory || 'Other Risk')}
                    </td>
                    <td className="border-r border-gray-200 p-4 text-gray-700">
                    ...
                    <td className="border-r border-gray-200 p-4 text-gray-700">
                      {riskType === 'physical' && renderArrayValue(risk.PotentialImpact)}
                      {riskType === 'transition' && renderArrayValue(risk.PotentialImpact)}
                      {riskType === 'other' && (risk.PotentialImpactoth || '-')}
                    </td>
                    <td className="border-r border-gray-200 p-4 text-gray-700">
                      {riskType === 'physical' && (risk.Likelihoodofimpact || '-')}
                      {riskType === 'transition' && (risk.Likelihoodofimpact || '-')}
                      {riskType === 'other' && '-'}
                    </td>
                    <td className="border-r border-gray-200 p-4 text-gray-700">
                    ...
                    <td className="border-r border-gray-200 p-4 text-gray-700">
                      {riskType === 'physical' && renderArrayValue(risk.ManagementMethods)}
                      {riskType === 'transition' && renderArrayValue(risk.ManagementMethods)}
                      {riskType === 'other' && (risk.ManagementMethodsoth || '-')}
                    </td>
                    <td className="border-r border-gray-200 p-4 text-gray-700">
                      {risk.TimeFrame || '-'}
                    </td>
                    ...
                    <td className="p-4 text-gray-700">
                      {riskType === 'physical' && (risk.MitigationStrategies || '-')}
                      {riskType === 'transition' && (risk.MitigationStrategies || '-')}
                      {riskType === 'other' && (risk.MitigationStrategiesoth || '-')}
                    </td>
                  </tr>
                ))}
    ...
    <>
      <div>
        <div id="section5_1" ref={section5_1Ref}>
          ...
          {/* Risk Tables */}
          <RiskTable title="Physical Risks" riskData={physicalRisks} riskType="physical" />
          <RiskTable title="Transition Risks" riskData={transitionRisks} riskType="transition" />
          <RiskTable title="Other Risks" riskData={otherRisks} riskType="other" />
