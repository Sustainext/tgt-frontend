...
const validateRows = (data) => {
  return data.map((row) => {
    ...
    if (!row.EnergyQuantity) {
      rowErrors.EnergyQuantity = "Energy Quantity is required";
    }
  
    if (!row.Unit) {
      rowErrors.Unit = "Unit is required";
    }
    if (!row.Organizationmetric) {
      rowErrors.Organizationmetric = "Organization Metric is required";
    }
    if (!row.Metricquantity) {
      rowErrors.Metricquantity = "Metric Quantity is required";
    }
    if (!row.Metricunit) {
      rowErrors.Metricunit = "Unit is required";
    }
    return rowErrors;
  });
};
