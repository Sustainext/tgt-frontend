  ...
  return (
    ...
    >
      ...
      <div className={`flex w-full xl:w-[75%] lg:w-[75%] md:w-[75%] 4k:w-[75%] 2xl:w-[75%]`}>
        <div className="flex w-[95%] xl:w-[95%] lg:w-[95%] md:w-[95%] 4k:w-[95%] 2xl:w-[95%] mb-2">
      ...
      {isOpen ? (
          <>
            {sdg && sdg.map((sdgItem, index) => (
              <div key={index} className="bg-sky-100 h-[25px] w-[80px]  text-center rounded-md mx-2" style={{ display: display }} >
          ...
          <>
            {sdg && sdg.map((sdgItem, index) => (
              <div key={index} className="bg-sky-100 h-[25px] w-[80px]  text-center rounded-md mx-2">
                <p className="text-[#0057A5] text-[10px] inline-block align-middle px-2 font-semibold">{sdgItem}</p>
              </div>
            ))}
          </>
  ...
  return (
    <div className="xl:mx-3 lg:mx-3 md:mx-3 2xl:mx-3 4k:mx-3 2k:mx-3 mx-1">
      <AccordionItem
        ...
        tooltiptext="<p>This section documents data corresponding to the total number of IUCN Red List species and national conservation list species with
habitats in areas affected by the operations of the organization, by level of extinction risk
</p>"
       
        sdg={["GRI 304-4a","GRI 304-4b"]}
        display="block"
