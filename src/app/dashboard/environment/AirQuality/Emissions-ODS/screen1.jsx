...
import EmissionsODSTable from "./emissionsODSTable";
...
const AccordionItem = ({ title, children, tooltiptext, sdg, display,location,setLocationMessage,year, setYearMessage  }) => {
      ...
      <div className={`shadow-md py-1 mb-4 rounded-[8px] cursor-pointer border border-b-3 border-neutral-200 `}>
        ...
        >
          ...
          <div className="flex items-center justify-center relative">
            <MdInfoOutline
              data-tooltip-id={`tooltip-${title.replace(/\s+/g, '-')}`} data-tooltip-content={tooltiptext} className="text-[14px]" style={{display:display}} />
            {/* Tooltip */}
  ...
  const Screen1 = ({location, year, month,setLocationMessage,setYearMessage}) => {
    ...
    return (
      <>
        ...
        <AccordionItem
            ...
            tooltiptext={`This section documents the data corresponding to the 
Emissions of ozone-depleting substances (ODS). 
Consumption(Data): 
In most cases, the data represents consumption in 
kilograms (kgs) or tons (e.g., refrigerant refill data)`}
            sdg={['GRI 305-6a','GRI 305-6b','GRI 305-6c']}
            display="block"
            location={location}
          ...
          >
            ...
            <EmissionsODSTable location={location} year={year} month={month}/>
          </AccordionItem>
