...
const Scope2 = ({ handleScope2Change, handleScope2Remove }) => {
  const { open } = GlobalState();
  const [formData, setFormData] = useState([{}]);

  const handleChange = (e) => {
    setFormData(e.formData);
    handleScope2Change(e.formData);
  };

  const handleAddNew = () => {
    const newData = [...formData, {}];
    setFormData(newData);
    handleScope2Change(newData);
  };
  ...
  const updateFormData = (newData) => {
    setFormData(newData);
    handleScope2Change(newData);
    handleScope2Remove(newData); // Ensure the parent state is updated
  };
  return (
    <>
