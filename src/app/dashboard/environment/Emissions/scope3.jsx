  ...
  return (
     ...
     {rows.map((row, index) => (
       <div key={index} className={`w-[100%]`}>
         <div className={`flex overflow-auto custom-scrollbar ${open ? "w-[790px]":"w-[950px]"}`}>
                 ...
                 if (property.format !== 'data-url') {
                   return (
                     ...
                     {property.enum && (key === 'Category' || key === 'Subcategory' || key === 'Activity') ? (
                       <select
                         className="block w-[152px] py-2 text-sm leading-6 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 sm:text-sm sm:leading-5 border-b-2 border-gray-300"
                         value={row[key] || ''}
                         onChange={(e) => {
                           const updatedRows = [...rows];
