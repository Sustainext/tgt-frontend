...
const Totalnumberemployees = ({selectedOrg, year,selectedCorp,activeMonth }) => {
                ...
                <div className='mb-4 flex mx-2 '>
                   <div className="w-[80%] relative">
                        ...
                        Total number of employees
                            <MdInfoOutline data-tooltip-id={`tooltip-$e1`} data-tooltip-content="This section documents data corresponding to the number of employees
by gender and geographic area, categorized by employment type." className="mt-1.5 ml-2 text-[14px]" />
                ...
                <div className="shadow-md rounded-md">
                    ...
                    <ul className="flex justify-evenly cursor-pointer">
                        {tabs.map((tab, index) => (
                            ...
                            >
                                ...
                                <ReactTooltip id={`tooltip-${tab.title.replace(/\s+/g, '-')}`} place="top" effect="solid" style={{
                                    width: "290px", backgroundColor: "#000",
                                    color: "white",
                                    fontSize: "12px",
                                    boxShadow: 3,
                    ...
                    {/* Content */}
                    <div className="mt-2">
                        {activeTabIndex === 0 && <div><Tab1 fullName={fullName} selectedOrg={selectedOrg} year={year} selectedCorp={selectedCorp} /></div>}
                        {activeTabIndex === 1 && <div><Tab2 fullName={fullName} selectedOrg={selectedOrg} year={year} selectedCorp={selectedCorp}  /> </div>}
                        {activeTabIndex === 2 && <div> <Tab3 fullName={fullName} selectedOrg={selectedOrg} year={year} selectedCorp={selectedCorp} /> </div>}
                        {activeTabIndex === 3 && <div> <Tab4 fullName={fullName} selectedOrg={selectedOrg} year={year} selectedCorp={selectedCorp} /> </div>}
                        {activeTabIndex === 4 && <div> <Tab5 fullName={fullName} selectedOrg={selectedOrg} year={year} selectedCorp={selectedCorp} /> </div>}
                    </div>
                </div>
            </div>
        </>
