...
const SubmissionInformation = ({
  ...
  return (
    <>
      {/* === Mobile Track Button === */}
      <button
        className=" xl:hidden lg:hidden md:hidden  bg-transparent text-gray-900 text-[13px]   border border-gray-300  rounded-md flex w-[100px] me-2 py-2 gap-1 px-2 text-center items-center justify-center mb-4 "
    ...
    <button onClick={() => setIsOpenMobile(false)} className="text-gray-600 hover:text-black">
      <MdClose className="text-2xl" />
    </button>
  </div>
        <div className="w-[285px] mt-6 mb-4 px-4 block xl:hidden lg:hidden md:hidden 2xl:hidden 4k:hideen">{renderStatusSteps()}</div>
      </div>

      {/* === Main Content Layout === */}
      <div className="h-[38rem] overflow-y-auto scrollable-content  xl:flex lg:flex md:flex 4k:flex 2xl:flex gap-6">
        {/* === Desktop Sidebar === */}
        <div className="w-[285px] mt-10 shadow-lg mb-4 hidden xl:block  lg:block md:block 2xl:block 4k:block">{renderStatusSteps()}</div>
