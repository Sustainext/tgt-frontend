...
const useActivityChanges = () => {
  ...
  const updateActivityChange = useCallback(
    (activityId, activityChange, saveToAPI = false, scenarioId = null) => {
      ...
      dispatch({
        type: 'optimise/setActivityChange',
        payload: { activityId, activityChange }
      });
      
      // Save to API if requested
      if (saveToAPI && scenarioId) {
        return dispatch({
          type: 'optimise/updateScenarioActivity',
          payload: {
            scenarioId,
            activityId,
  ...
  const updateChangesInActivity = useCallback(
    (activityId, changesInActivity, saveToAPI = false, scenarioId = null) => {
      ...
      dispatch({
        type: 'optimise/setChangesInActivity',
        payload: { activityId, changesInActivity }
      });
  ...
  const handleActivityGraphChange = useCallback(
    (activityId, changes, saveToAPI = false, scenarioId = null) => {
      const { activity_change, changes_in_activity } = changes;
      
      // Gather all properties to update
      const propertiesToUpdate = {};
      
      // Always include activity_change (even if it's false)
      propertiesToUpdate.activity_change = activity_change;
      
      // Only include changes_in_activity if it's provided and not empty
      if (changes_in_activity && Object.keys(changes_in_activity).length > 0) {
        propertiesToUpdate.changes_in_activity = changes_in_activity;
      }
      
      // Use the updateActivity function to update all properties at once
      return updateActivity(activityId, propertiesToUpdate, saveToAPI, scenarioId);
    },
    [updateActivity]
  );
